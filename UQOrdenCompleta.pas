...
begin
  Value:= Value + ' - ' + DDescripciones.desEnvaseP( DataSet.FieldByName('empresa_occ').AsString, Value,
...
procedure TQOrdenCompleta.QRDBText44Print(sender: TObject;
  var Value: String);
begin
  Value := DDescripciones.desEnvaseProducto( bndEnvases.DataSet.FieldByname('empresa_pl').AsString, Value,
...
//    Value:= 'SIN MARCA';
    if bndLineas.DataSet.Active then
      Value:= DDescripciones.desCategoria( DataSet.FieldByName('empresa_occ').AsString,
                                           bndLineas.DataSet.FieldByName('producto_ocl').AsString,
                                           Value)
    else
      Value := '';
